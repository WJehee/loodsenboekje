...
#[cfg(feature="ssr")]
pub async fn get_user_by_username(username: &str) -> Result<SqlUser, ServerFnError> {
    let db = db().await?;
    sqlx::query_as!(SqlUser, "SELECT * FROM users WHERE username = ?", username)
        .fetch_one(&db)
        .await
        .map_err(|_| Error::Database.into())
}

#[cfg(feature="ssr")]
pub async fn get_user_by_id(id: i64) -> Result<SqlUser, ServerFnError> {
    let db = db().await?;
